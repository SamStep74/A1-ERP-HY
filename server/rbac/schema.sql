...
CREATE INDEX IF NOT EXISTS idx_rbac_roles_parent ON rbac_roles(parent);
...
CREATE TABLE IF NOT EXISTS rbac_role_permission_sets (
...
CREATE INDEX IF NOT EXISTS idx_rbac_rps_role ON rbac_role_permission_sets(role_id);
CREATE INDEX IF NOT EXISTS idx_rbac_rps_set  ON rbac_role_permission_sets(permission_set_id);
...
CREATE INDEX IF NOT EXISTS idx_rbac_perm_category ON rbac_permissions(category);
CREATE INDEX IF NOT EXISTS idx_rbac_perm_tenant ON rbac_permissions(tenant_id);

-- ───────────── Permission sets ─────────────

CREATE TABLE IF NOT EXISTS rbac_permission_sets (
...
CREATE INDEX IF NOT EXISTS idx_rbac_ps_tenant ON rbac_permission_sets(tenant_id);

-- Members of a permission set. Frozen at creation in code; UI may add/remove
-- within a tenant.
CREATE TABLE IF NOT EXISTS rbac_permission_set_members (
  permission_set_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_rbac_psm_set ON rbac_permission_set_members(permission_set_id);
CREATE INDEX IF NOT EXISTS idx_rbac_psm_key ON rbac_permission_set_members(permission_key);

-- ───────────── User direct assignments ─────────────

-- Direct user → permission set grants (in addition to role defaults).
CREATE TABLE IF NOT EXISTS rbac_user_permission_sets (
...
CREATE INDEX IF NOT EXISTS idx_rbac_ups_user ON rbac_user_permission_sets(user_id);
...
CREATE TABLE IF NOT EXISTS rbac_record_rules (
...
CREATE INDEX IF NOT EXISTS idx_rbac_sess_user ON rbac_sessions(user_id);
CREATE INDEX IF NOT EXISTS idx_rbac_sess_tenant ON rbac_sessions(tenant_id);
CREATE INDEX IF NOT EXISTS idx_rbac_sess_active ON rbac_sessions(revoked_at, expires_at);
...
CREATE TABLE IF NOT EXISTS rbac_approvals (
...
CREATE INDEX IF NOT EXISTS idx_rbac_app_tenant ON rbac_approvals(tenant_id);
CREATE INDEX IF NOT EXISTS idx_rbac_app_status ON rbac_approvals(status);
...
CREATE INDEX IF NOT EXISTS idx_rbac_audit_user ON rbac_permission_audit(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_rbac_audit_perm ON rbac_permission_audit(permission, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_rbac_audit_tenant ON rbac_permission_audit(tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_rbac_audit_decision ON rbac_permission_audit(decision, created_at DESC);

-- ───────────── Impersonation log ─────────────

CREATE TABLE IF NOT EXISTS rbac_impersonation_log (
  id            INTEGER PRIMARY KEY AUTOINCREMENT,
  tenant_id     INTEGER NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_rbac_imp_actor ON rbac_impersonation_log(actor_id, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_rbac_imp_target ON rbac_impersonation_log(target_id, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_rbac_imp_tenant ON rbac_impersonation_log(tenant_id, started_at DESC);
...
INSERT OR IGNORE INTO rbac_meta (key, value) VALUES
