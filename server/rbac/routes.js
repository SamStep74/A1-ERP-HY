...
const {
  hasPermission, requirePerm, requireAnyPerm, resolveEffectivePermissions, redactFields,
} = require('./guards');
...
function registerRbacRoutes(app, opts = {}) {
  ...
  app.get('/api/rbac/permissions', { preHandler: requirePerm('security.permission_set.read') }, async (request) => {
    return {
  ...
  app.get('/api/rbac/permissions/:key', { preHandler: requirePerm('security.permission_set.read') }, async (request, reply) => {
  ...
  app.get('/api/rbac/permission-sets', { preHandler: requirePerm('security.permission_set.read') }, async () => {
    return {
      version: PERMISSION_SETS_VERSION || 1,
      items: Object.values(PERMISSION_SETS).map(ps => ({
        id: ps.id, label: ps.label, description: ps.description,
        isSystem: !!ps.isSystem, memberCount: ps.permissions.length,
      })),
    };
  });

  app.get('/api/rbac/permission-sets/:id', { preHandler: requirePerm('security.permission_set.read') }, async (request, reply) => {
  ...
  app.get('/api/rbac/roles', { preHandler: requirePerm('security.role.read') }, async () => {
    const items = listRoleIds().map(id => {
      const r = ROLES[id];
      return {
        id, label: r.label, description: r.description, parent: r.parent,
        isSystem: !!r.isSystem, mfaRequired: !!r.mfaRequired,
        sessionHardLimitMinutes: r.sessionHardLimitMinutes,
        canBeImpersonated: !!r.canBeImpersonated,
        appSet: r.appSet, defaultPermissionSets: listForRole(id),
      };
    });
    return { items };
  });

  app.post('/api/rbac/roles', { preHandler: requirePerm('security.role.create') }, async (request, reply) => {
    ...
    `);
    stmt.run(
      validated.id, validated.label, validated.description, validated.parent,
      JSON.stringify(validated.appSet), validated.mfaRequired ? 1 : 0,
      validated.sessionHardLimitMinutes, validated.canBeImpersonated ? 1 : 0,
      request.user.id,
    );
    return reply.code(201).send(validated);
  });

  app.patch('/api/rbac/roles/:id', { preHandler: requirePerm('security.role.update') }, async (request, reply) => {
    const id = request.params.id;
    const r = getRole(id);
    if (!r) return reply.code(404).send({ error: 'not_found' });
    if (r.isSystem) {
      // System roles are mostly read-only; only appSet/description mutable.
      const allowed = ['label', 'description', 'appSet', 'mfaRequired', 'sessionHardLimitMinutes', 'canBeImpersonated'];
      ...
      `).run(
        next.label, next.description, JSON.stringify(next.appSet),
        next.mfaRequired ? 1 : 0, next.sessionHardLimitMinutes,
        next.canBeImpersonated ? 1 : 0, id,
      );
      return { ...next };
    }
    ...
    db.prepare(`
      UPDATE rbac_roles
       ...
       WHERE id = ?
    `).run(
      merged.label, merged.description, merged.parent, JSON.stringify(merged.appSet),
      merged.mfaRequired ? 1 : 0, merged.sessionHardLimitMinutes,
      merged.canBeImpersonated ? 1 : 0, id,
    );
    return merged;
  });

  app.delete('/api/rbac/roles/:id', { preHandler: requirePerm('security.role.delete') }, async (request, reply) => {
    const id = request.params.id;
    const r = getRole(id);
    if (!r) return reply.code(404).send({ error: 'not_found' });
    if (r.isSystem) return reply.code(409).send({ error: 'system_role_immutable' });
    // Refuse if anyone still holds this role.
    const used = db.prepare(`SELECT COUNT(*) AS c FROM rbac_user_roles WHERE role_id = ?`).get(id);
    if (used && used.c > 0) return reply.code(409).send({ error: 'role_in_use', count: used.c });
    db.prepare(`DELETE FROM rbac_roles WHERE id = ? AND is_system = 0`).run(id);
    return reply.code(204).send();
  });

  // ───── User effective permissions ─────

  app.get('/api/rbac/users/:userId/effective', { preHandler: requirePerm('security.user.read') }, async (request, reply) => {
    ...
    const directPS = db.prepare(`
      SELECT permission_set_id FROM rbac_user_permission_sets
       WHERE user_id = ? AND tenant_id = ?
    `).all(userId, u.tenant_id || 0).map(r => r.permission_set_id);

    const user = {
      id: u.id, role: u.primary_role || 'SalesRep', permission_set_ids: directPS,
      tenant_id: u.tenant_id, org_id: u.org_id,
      mfa_required: !!u.mfa_required, mfa_verified: !!u.mfa_verified,
    };
  ...
  app.post('/api/rbac/users/:userId/permission-sets', { preHandler: requirePerm('security.role.assign') }, async (request, reply) => {
    const userId = Number(request.params.userId);
    const body = request.body || {};
    ...
    db.prepare(`
      INSERT INTO rbac_user_permission_sets (user_id, permission_set_id, tenant_id, granted_by, expires_at)
      VALUES (?, ?, ?, ?, ?)
  ...
  app.delete('/api/rbac/users/:userId/permission-sets/:ps', { preHandler: requirePerm('security.role.assign') }, async (request, reply) => {
    const userId = Number(request.params.userId);
    const psId = String(request.params.ps);
    const tenant = db.prepare(`SELECT tenant_id FROM users WHERE id = ?`).get(userId);
    if (!tenant) return reply.code(404).send({ error: 'user_not_found' });
    db.prepare(`DELETE FROM rbac_user_permission_sets WHERE user_id = ? AND permission_set_id = ? AND tenant_id = ?`)
      .run(userId, psId, tenant.tenant_id || 0);
    return reply.code(204).send();
  });

  app.post('/api/rbac/users/:userId/role', { preHandler: requirePerm('security.role.assign') }, async (request, reply) => {
    const userId = Number(request.params.userId);
    const body = request.body || {};
    ...
    if (!tenant) return reply.code(404).send({ error: 'user_not_found' });
    db.prepare(`
      INSERT INTO rbac_user_roles (user_id, role_id, tenant_id, assigned_by)
      VALUES (?, ?, ?, ?)
    ...
    } catch (_) { /* users.role may not exist in some deployments */ }
    return reply.code(201).send({ userId, roleId });
  });

  // ───── Field-level security ─────

  app.get('/api/rbac/field-policies', { preHandler: requirePerm('security.permission_set.read') }, async () => {
    return {
      items: db.prepare(`SELECT field_path, min_permission, is_visible, label, updated_at
                            FROM rbac_field_policies WHERE tenant_id = 0
                            ORDER BY field_path`).all(),
    };
  });

  app.put('/api/rbac/field-policies/:path(*)', { preHandler: requirePerm('security.permission_set.update') }, async (request, reply) => {
    ...
    db.prepare(`
      INSERT INTO rbac_field_policies (field_path, tenant_id, min_permission, is_visible, label, updated_by)
      VALUES (?, 0, ?, ?, ?, ?)
      ON CONFLICT(tenant_id, field_path) DO UPDATE SET
        ...
        updated_at = datetime('now')
    `).run(path, body.minPermission, body.isVisible === false ? 0 : 1, body.label || '', request.user.id);
    return reply.code(200).send({ fieldPath: path, ...body });
  });

  // ───── Record-level security ─────

  app.get('/api/rbac/record-rules', { preHandler: requirePerm('security.permission_set.read') }, async () => {
    return {
      items: db.prepare(`SELECT resource, scope, predicate, description, updated_at
                            FROM rbac_record_rules WHERE tenant_id = 0
                            ORDER BY resource`).all(),
    };
  });

  app.put('/api/rbac/record-rules/:resource(*)', { preHandler: requirePerm('security.permission_set.update') }, async (request, reply) => {
    const resource = String(request.params.resource || '');
    const body = request.body || {};
    const validScopes = ['own', 'team', 'org', 'custom'];
    if (!validScopes.includes(body.scope)) return reply.code(400).send({ error: 'invalid_scope' });
    if (body.scope === 'custom' && (typeof body.predicate !== 'string' || !body.predicate.trim())) {
      return reply.code(400).send({ error: 'predicate_required_for_custom_scope' });
    }
    db.prepare(`
      INSERT INTO rbac_record_rules (resource, tenant_id, scope, predicate, description, updated_by)
      VALUES (?, 0, ?, ?, ?, ?)
      ON CONFLICT(tenant_id, resource) DO UPDATE SET
        ...
        updated_by = excluded.updated_by,
        updated_at = datetime('now')
    `).run(resource, body.scope, body.scope === 'custom' ? body.predicate : null, body.description || '', request.user.id);
    return reply.code(200).send({ resource, ...body });
  });

  // ───── Sessions ─────

  app.get('/api/rbac/sessions', { preHandler: requirePerm('security.session.list') }, async (request) => {
  ...
  app.delete('/api/rbac/sessions/:id', { preHandler: requirePerm('security.session.revoke') }, async (request, reply) => {
    db.prepare(`UPDATE rbac_sessions SET revoked_at = datetime('now') WHERE id = ?`).run(request.params.id);
    return reply.code(204).send();
  });

  // ───── Audit ─────

  app.get('/api/rbac/audit', { preHandler: requirePerm('security.audit.read') }, async (request) => {
    ...
    if (request.query.userId) { where += ` AND user_id = ?`; params.push(Number(request.query.userId)); }
    return {
      items: db.prepare(`
        SELECT id, user_id, permission, decision, resource, reason, ip, session_id, created_at
          FROM rbac_permission_audit WHERE ${where}
  ...
  app.get('/api/rbac/me/permissions', { preHandler: (req, _rep, done) => done() }, async (request) => {
  ...
  app.get('/api/rbac/health', { preHandler: requireAnyPerm(['security.role.read','security.permission_set.read','security.audit.read']) }, async () => {
    const issues = [];
    // Each role in the matrix should have a defined role object.
    for (const r of Object.keys(ROLE_MATRIX)) {
      if (!ROLES[r]) issues.push({ level: 'error', msg: `role_matrix_references_unknown_role: ${r}` });
    }
    ...
    for (const ps of Object.values(PERMISSION_SETS)) {
      for (const k of ps.permissions) {
        if (!PERMISSIONS[k]) issues.push({ level: 'error', msg: `permission_set_references_unknown_permission: ${ps.id} -> ${k}` });
      }
    }
