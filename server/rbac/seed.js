...
async function seedPermissions(db) {
  const stmt = db.prepare(`
    INSERT INTO rbac_permissions (key, tenant_id, category, sensitivity, label, description, is_system, updated_at)
  ...
  db.prepare(`UPDATE rbac_meta SET value = ?, updated_at = datetime('now') WHERE key = 'permissions_version'`)
...
async function seedRoles(db) {
  ...
  db.prepare(`UPDATE rbac_meta SET value = ?, updated_at = datetime('now') WHERE key = 'roles_version'`)
    .run(String(ROLES_VERSION));
}

async function seedPermissionSets(db) {
  const psStmt = db.prepare(`
    INSERT INTO rbac_permission_sets (id, tenant_id, label, description, is_system, updated_at)
    VALUES (?, 0, ?, ?, 1, datetime('now'))
    ON CONFLICT(id) DO UPDATE SET
      label = excluded.label,
      description = excluded.description
  ...
  db.prepare(`UPDATE rbac_meta SET value = ?, updated_at = datetime('now') WHERE key = 'permission_sets_version'`)
    .run(String(PERMISSION_SETS_VERSION));

  // Members
  const memberStmt = db.prepare(`
    INSERT INTO rbac_permission_set_members (permission_set_id, permission_key, tenant_id)
    VALUES (?, ?, 0)
...
async function seedRolePermissionSets(db) {
  const stmt = db.prepare(`
    INSERT INTO rbac_role_permission_sets (role_id, permission_set_id, tenant_id)
    VALUES (?, ?, 0)
...
async function seedRBAC(db, opts = {}) {
  ...
  if (opts.force) {
    ...
    // rows are preserved.
    db.exec(`
      DELETE FROM rbac_permission_set_members WHERE tenant_id = 0;
      DELETE FROM rbac_permission_sets WHERE tenant_id = 0;
      DELETE FROM rbac_role_permission_sets WHERE tenant_id = 0;
      DELETE FROM rbac_roles WHERE tenant_id = 0;
      DELETE FROM rbac_permissions WHERE tenant_id = 0;
    `);
  }
...
function readVersions(db) {
  const rows = db.prepare(`SELECT key, value FROM rbac_meta`).all();
  const out = {};
